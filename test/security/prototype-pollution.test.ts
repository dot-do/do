...
describe('Prototype Pollution Prevention', () => {
  ...
  describe.todo('__proto__ injection prevention', () => {
    it('should block direct __proto__ assignment on objects', async () => {
      ...
      const result = await stub.do('const obj = {}; obj.__proto__.polluted = true; return ({}).polluted')
    ...
    it('should block __proto__ in object literals', async () => {
      ...
      const result = await stub.do('const obj = { "__proto__": { polluted: true } }; return ({}).polluted')
      expect(result.success === false || result.result === undefined).toBe(true)
    })
    ...
    it('should block __proto__ in nested objects', async () => {
      ...
      const result = await stub.do('const obj = { nested: { "__proto__": { polluted: true } } }; return ({}).polluted')
      expect(result.success === false || result.result === undefined).toBe(true)
    })
    ...
    it('should block string concatenation to build __proto__', async () => {
      ...
      const result = await stub.do('const key = "__" + "proto" + "__"; const obj = {}; obj[key].polluted = true; return ({}).polluted')
      expect(result.success === false || result.result === undefined).toBe(true)
    })
  })
  ...
  describe.todo('Constructor.prototype protection', () => {
    it('should block constructor.prototype modification via instance', async () => {
      ...
      const result = await stub.do('const obj = {}; obj.constructor.prototype.polluted = true; return ({}).polluted')
      expect(result.success === false || result.result === undefined).toBe(true)
    })

    it('should block array constructor.prototype pollution', async () => {
      // Attacker accesses Array constructor.prototype via instance
      const result = await stub.do('const arr = []; arr.constructor.prototype.polluted = true; return [].polluted')
      expect(result.success === false || result.result === undefined).toBe(true)
    })

    it('should block string constructor.prototype pollution', async () => {
      // Attacker accesses String constructor.prototype via instance
      const result = await stub.do('const str = ""; str.constructor.prototype.polluted = true; return "".polluted')
      expect(result.success === false || result.result === undefined).toBe(true)
    })

    it('should block chained constructor access', async () => {
      // Attacker uses chained constructor access
      const result = await stub.do('({}).constructor.constructor.prototype.polluted = true; return ({}).polluted')
      expect(result.success === false || result.result === undefined).toBe(true)
    })
  ...
  describe.todo('Nested prototype pollution', () => {
    it('should block prototype pollution via nested object merge', async () => {
      // Common vulnerability in merge/extend functions - blocked by __proto__ detection in JSON string
      const result = await stub.do('function merge(t,s){for(const k in s){if(typeof s[k]==="object"){t[k]=t[k]||{};merge(t[k],s[k])}else{t[k]=s[k]}}return t};const p=JSON.parse(\'{"__proto__":{"polluted":true}}\');merge({},p);return ({}).polluted')
      expect(result.success === false || result.result === undefined).toBe(true)
    })

    it('should block deeply nested __proto__ pollution', async () => {
      // Attacker uses deeply nested structure
      const result = await stub.do('const obj = { a: { b: { c: {} } } }; obj.a.b.c.__proto__.polluted = true; return ({}).polluted')
      expect(result.success === false || result.result === undefined).toBe(true)
    })

    it('should block prototype pollution via spread operator workarounds', async () => {
      // Attacker tries to use spread operator
      const result = await stub.do('const malicious = { "__proto__": { polluted: true } }; const obj = { ...malicious }; return ({}).polluted')
      expect(result.success === false || result.result === undefined).toBe(true)
    })

    it('should block Object.assign with __proto__ key', async () => {
      // Attacker uses Object.assign with __proto__
      const result = await stub.do('const target = {}; Object.assign(target, { "__proto__": { polluted: true } }); return ({}).polluted')
      expect(result.success === false || result.result === undefined).toBe(true)
    })
  })
  ...
  describe.todo('JSON.parse prototype pollution', () => {
    it('should block JSON.parse with __proto__ key', async () => {
      // Classic JSON.parse prototype pollution vector - blocked by __proto__ in string
      const result = await stub.do('const parsed = JSON.parse(\'{"__proto__":{"polluted":true}}\'); return ({}).polluted')
      expect(result.success === false || result.result === undefined).toBe(true)
    })

    it('should block JSON.parse with nested __proto__', async () => {
      // Nested __proto__ in JSON - blocked by __proto__ in string
      const result = await stub.do('const json = \'{"nested":{"__proto__":{"polluted":true}}}\'; const parsed = JSON.parse(json); return ({}).polluted')
      expect(result.success === false || result.result === undefined).toBe(true)
    })

    it('should block JSON.parse with constructor pollution', async () => {
      // JSON with constructor.prototype path - blocked by constructor["prototype"] pattern
      const result = await stub.do('const json = \'{"constructor":{"prototype":{"polluted":true}}}\'; const parsed = JSON.parse(json); return ({}).polluted')
      expect(result.success === false || result.result === undefined).toBe(true)
    })
    ...
    it('should safely parse legitimate JSON with __proto__ as a string value', async () => {
      ...
      // This will be blocked because the code contains __proto__ string
      const result = await stub.do('const json = \'{"key":"__proto__"}\'; const parsed = JSON.parse(json); return parsed.key')
      // This will be blocked because __proto__ appears in the code string
      expect(result.success === false || result.result === '__proto__').toBe(true)
    })
    ...
    it('should use safe JSON.parse reviver to filter dangerous keys', async () => {
      ...
      // This will be blocked because the code contains __proto__ string
      const result = await stub.do('const json = \'{"__proto__":{"polluted":true},"safe":"value"}\'; const parsed = JSON.parse(json); return { hasSafe: parsed.safe === "value", isPolluted: ({}).polluted }')
      // Blocked by static analysis due to __proto__ in code
      expect(result.success === false || (result.result as { isPolluted: unknown })?.isPolluted === undefined).toBe(true)
    })
  })

  describe.todo('Object.assign pollution vectors', () => {
    it('should block Object.assign with __proto__ source', async () => {
      // Object.assign can be used for prototype pollution - blocked by __proto__ in string
      const result = await stub.do('const source = JSON.parse(\'{"__proto__":{"polluted":true}}\'); Object.assign({}, source); return ({}).polluted')
      expect(result.success === false || result.result === undefined).toBe(true)
    })

    it('should block Object.assign to Object.prototype', async () => {
      // Direct assignment to Object.prototype
      const result = await stub.do('Object.assign(Object.prototype, { polluted: true }); return ({}).polluted')
      expect(result.success === false || result.result === undefined).toBe(true)
    })
    ...
    it('should block Object.assign with constructor.prototype target', async () => {
      ...
      const result = await stub.do('Object.assign(({}).constructor.prototype, { polluted: true }); return ({}).polluted')
      expect(result.success === false || result.result === undefined).toBe(true)
    })

    it('should block multiple Object.assign calls building pollution', async () => {
      // Chained assignments that eventually pollute - blocked by __proto__ in string
      const result = await stub.do('const base = {}; const ext1 = { nested: {} }; const ext2 = { nested: { "__proto__": { polluted: true } } }; Object.assign(base, ext1); Object.assign(base.nested, ext2.nested); return ({}).polluted')
      expect(result.success === false || result.result === undefined).toBe(true)
    })

    it('should allow safe Object.assign usage', async () => {
      // Legitimate Object.assign should work
      const result = await stub.do('const target = { a: 1 }; const source = { b: 2 }; const merged = Object.assign(target, source); return merged')
      expect(result.success).toBe(true)
      expect(result.result).toEqual({ a: 1, b: 2 })
    })

    it('should block Object.assign with computed __proto__ key', async () => {
      // Attacker computes the __proto__ key dynamically - blocked by string concat detection
      const result = await stub.do('const key = "__" + "proto" + "__"; const source = {}; source[key] = { polluted: true }; Object.assign({}, source); return ({}).polluted')
      expect(result.success === false || result.result === undefined).toBe(true)
    })
  })
  ...
  describe.todo('Additional prototype pollution vectors', () => {
    it('should block Object.setPrototypeOf for pollution', async () => {
      ...
      const result = await stub.do('const malicious = { polluted: true }; Object.setPrototypeOf({}, malicious); return ({}).polluted')
    ...
    it('should block Reflect.setPrototypeOf', async () => {
      ...
      const result = await stub.do('Reflect.setPrototypeOf({}, { polluted: true }); return ({}).polluted')
      expect(result.success === false || result.result === undefined).toBe(true)
    })

    it('should block Object.getPrototypeOf for chain traversal', async () => {
      // Attacker uses getPrototypeOf to traverse and modify chain
      const result = await stub.do('const proto = Object.getPrototypeOf({}); proto.polluted = true; return ({}).polluted')
      expect(result.success === false || result.result === undefined).toBe(true)
    })

    it('should block Reflect.getPrototypeOf for chain traversal', async () => {
      // Attacker uses Reflect.getPrototypeOf
      const result = await stub.do('const proto = Reflect.getPrototypeOf({}); proto.polluted = true; return ({}).polluted')
      expect(result.success === false || result.result === undefined).toBe(true)
    })

    it('should block prototype pollution via eval', async () => {
      // Attacker uses eval to bypass static analysis
      const result = await stub.do('eval("Object.prototype.polluted = true"); return ({}).polluted')
    ...
    it('should block prototype pollution via Function constructor', async () => {
      ...
      const result = await stub.do('new Function("Object.prototype.polluted = true")(); return ({}).polluted')
