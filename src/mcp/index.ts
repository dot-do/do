...
export class McpHandler {
  ...
  private async handleToolCall(toolName: string, request: Request): Promise<Response> {
    ...
    try {
      ...
      switch (toolName) {
        case 'search':
          const searchResult = await this.target.search(body.query as string, {
        ...
        case 'fetch':
          const fetchResult = await this.target.fetch(body.target as string)
          return Response.json({ result: fetchResult })

        case 'do':
          const doResult = await this.target.do(body.code as string, {
